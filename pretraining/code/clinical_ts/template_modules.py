...
class BaseConfigData:
    ...
    train_fold_ids:List[str]=field(default_factory=lambda: ["xm1","xm2"]) # can use number to select folds (using an m allows to select folds from backwards i.e. m1 for the last fold etc), using a x as prefix excludes a fold that follows the x
    val_fold_ids:List[str]=field(default_factory=lambda: ["m2"])
    test_fold_ids:List[str]=field(default_factory=lambda: ["m1"])
...
class TimeSeriesEncoder(EncoderBase):
    ...
    def __init__(self, hparams_seqenc, hparams_input_shape, static_stats_train, target_dim=None, has_ema=True):
        ...
        if hparams_seqenc.loss.loss_type.startswith("instance_contrastive"):
            ...
            ssl_target_dim = np.sum(hparams_seqenc.loss.kmeans_ks)*len(hparams_seqenc.ema.module_name) #just one combined linear layer for all codebooks and heads to be taken apart in the loss function
        elif(hparams_seqenc.loss.loss_type.startswith("masked_pred_capi") or hparams_seqenc.loss.loss_type.startswith("masked_pred_skkmeans")):
            ssl_target_dim = np.sum(hparams_seqenc.loss.kmeans_ks)*len(hparams_seqenc.ema.module_name) #just one combined linear layer for all codebooks and heads to be taken apart in the loss function
        elif hparams_seqenc.loss.loss_type in ["data2vec", "jepa"]:
            ssl_target_dim = hparams_seqenc.pred.model_dim
        elif hparams_seqenc.loss.loss_type == "dinosr":
            ssl_target_dim = np.sum(hparams_seqenc.loss.kmeans_ks) * len(hparams_seqenc.ema.module_name)
    ...
    def forward(self, **kwargs): 
        ...
        if(self.loss is None): #standard supervised forward
            ...
            if(self.masking is not None):
                ...
                # JEPA Masking
                if "masks_pred" in masking_res:
                    seq_enc_w_static_enc2["masks_pred"] = masking_res["masks_pred"]
                if "masks_enc" in masking_res:
                    seq_enc_w_static_enc2["masks_enc"] = masking_res["masks_enc"]
...
class EMATimeSeriesEncoderBaseConfig:
    ...
    momentum:float = 0.9996 #EMA momentum parameter (very stable by default)
    ...
    update_every_k_batches:int = 1

    #targets
    module_name:List[str] = field(default_factory=lambda: ["predictor"]) #also supports [i] for module lists such as predictor.predictor.s4_layers[1]
    transpose_axes_after_hook:List[List[int]]=field(default_factory=lambda: [])#can pass a permutation order e.g. [[0,2,1]] for intermediate s4 layers; currently the same for all hooks
