...
class Main_Lite(lp.LightningModule):
    ...
    #             param.data = state_dict[name].data.to(param.device)
    #         elif strict:
    #             if self.hparams.architecture == "xecg":
    #                 if not any(x in name for x in ["causal_mask", "mlstm_cell", "slstm_cell"]):
    #                     raise KeyError(f"Buffer {name} not found in state_dict")
    #             else:
    #                 raise KeyError(f"Buffer {name} not found in state_dict")

    def load_state_dict(self, state_dict, strict=True):
        # Track what we're skipping for transparency
        skipped_buffers = []
        ...
        # Load buffers with selective strictness
        for name, param in self.named_buffers():
            if name in state_dict:
                param.data = state_dict[name].data.to(param.device)
            elif strict:
                # Define buffers that are safe to skip (auto-generated)
                safe_to_skip = ['cos_cached', 'sin_cached']
                
                # Check if this is a safe-to-skip buffer
                if any(skip_pattern in name for skip_pattern in safe_to_skip):
                    skipped_buffers.append(name)
                    continue  # Skip without error
                
                # Apply original strict logic for other buffers
                if self.hparams.architecture == "xecg":
                    if not any(x in name for x in ["causal_mask", "mlstm_cell", "slstm_cell"]):
                        raise KeyError(f"Buffer {name} not found in state_dict")
                else:
        ...
        # Log what was skipped (optional but helpful)
        if skipped_buffers:
            print(f"Info: Skipped {len(skipped_buffers)} auto-generated buffer(s): {skipped_buffers}")