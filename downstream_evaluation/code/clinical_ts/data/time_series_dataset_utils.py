__all__ = [ 'migrate_pickle_to_parquet', 'check_and_clean_numpy_files','save_dataset', 'load_dataset','dataset_add_chunk_col', 'dataset_add_length_col', 'dataset_add_labels_col', 'dataset_add_mean_col', 'dataset_add_median_col', 'dataset_add_std_col', 'dataset_add_iqr_col', 'dataset_get_stats', 'append_to_memmap', 'append_to_df_memmap', 'upgrade_memmap_meta', 'npys_to_memmap_batched', 'npys_to_memmap', 'reformat_as_memmap', 'load_memmap_meta_dict']
...
def migrate_pickle_to_parquet(pickle_path, parquet_path=None):
    """Convert pandas pickle files to parquet format"""
    if parquet_path is None:
        parquet_path = pickle_path.replace('.pkl', '.parquet')
    
    # Read pickle
    df = pd.read_pickle(pickle_path)
    
    # Save as parquet with optimal settings
    df.to_parquet(
        parquet_path,
        engine='pyarrow',          # Fast engine
        compression='snappy',      # Good balance of speed/size
        index=False,               # Usually don't need index
        partition_cols=None        # Can partition for huge datasets
    )
    
    # Compare sizes
    pkl_size = os.path.getsize(pickle_path) / 1024**2
    parquet_size = os.path.getsize(parquet_path) / 1024**2
    
    print(f"Converted {pickle_path}")
    print(f"Size reduction: {pkl_size:.1f} MB → {parquet_size:.1f} MB")
    print(f"Reduction: {pkl_size/parquet_size:.1f}x smaller")
...
def save_dataset(df,lbl_itos=None,mean=None,std=None,target_root=".",df_filename="df.parquet",protocol=4):
    target_root = Path(target_root)
    if(df_filename.endswith("pkl")):
        df.to_pickle(target_root/df_filename, protocol=protocol)
    elif(df_filename.endswith("parquet")):
        df.to_parquet(target_root/df_filename)
    else:
        assert(False)
...
def load_dataset(target_root,df_filename="df_memmap.parquet"):
    target_root = Path(target_root)

    # Check if the specified file exists
    file_path = target_root / df_filename
    
    if not file_path.exists():
        # For backwards compatibility: if default doesn't exist, try with .pkl extension using the same stem
        if df_filename.endswith(".parquet"):
            pkl_path = target_root / df_filename.replace(".parquet", ".pkl")
            if pkl_path.exists():
                file_path = pkl_path
                df_filename = pkl_path.name
            else:
                raise FileNotFoundError(f"Neither {df_filename} nor {pkl_path.name} found in {target_root}")
        else:
            raise FileNotFoundError(f"File {df_filename} not found in {target_root}")
    
    # Read based on file extension
    if df_filename.endswith(".parquet"):
        df = pd.read_parquet(file_path)
    elif df_filename.endswith(".pkl"):
        df = pd.read_pickle(file_path)
    else:
        raise ValueError(f"Unsupported file format: {df_filename}")
...
def reformat_as_memmap(df, target_filename, fs, channel_itos=None, fs_annotation=None, channel_itos_annotation=None, data_folder=None, annotation=False, max_len=900000000, delete_npys=True, col_data="data", col_lbl="label", batch_length=0, skip_export_signals=False, df_as_parquet=True):
    ...
    if(df_as_parquet):    
        df_mapped.to_parquet(target_filename.parent/("df_"+target_filename.stem+".parquet"))
    else:
